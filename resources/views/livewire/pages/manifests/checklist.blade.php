    ...
    <div class="relative z-10 max-w-7xl mx-auto space-y-5">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
                            ...
                            <button wire:key="category-{{ $category->id }}"
                                ...
                                <div class="flex items-center gap-3">
                                    <div
                                        ...
                                        @if ($isApproved)
                                            <x-heroicon-s-check-badge wire:loading.remove
                                                wire:target="selectCategory({{ $category->id }})"
                                                class="w-4 h-4 text-emerald-500" />
                                        @elseif($isRejected)
                                            <x-heroicon-s-x-circle wire:loading.remove
                                                wire:target="selectCategory({{ $category->id }})"
                                                class="w-4 h-4 text-rose-500" />
                                        @else
                                            <x-heroicon-o-queue-list wire:loading.remove
                                                wire:target="selectCategory({{ $category->id }})"
                                                class="w-4 h-4 {{ $isActive ? 'text-white' : 'text-zinc-400 dark:text-zinc-500' }}" />
                                        @endif
                                        <span wire:loading wire:target="selectCategory({{ $category->id }})"
                                            class="loading loading-spinner loading-xs {{ $isActive ? 'text-white' : 'text-indigo-600' }}"></span>
                                    </div>
                                    <div class="text-left">
            ...
            <div
                ...
                <!-- Items List -->
                <div class="p-4 flex-1 space-y-3 overflow-y-auto relative">
                    <div wire:loading.class="opacity-50 blur-[2px] pointer-events-none"
                        class="transition-all duration-300 absolute inset-0 z-10 hidden"
                        wire:loading.class.remove="hidden"></div>
                    <div wire:loading.class="opacity-50 blur-[2px] pointer-events-none"
                        class="transition-all duration-300 space-y-3">
                                ...
                                <div wire:key="item-{{ $item->id }}"
                                    ...
                                    <div class="flex flex-col gap-3">
                                        ...
                                        @if ($resultVal === 'pass_with_note' || $resultVal === 'fail')
                                            <div
                                                class="pt-3 border-t border-dashed border-zinc-100 dark:border-zinc-800">
